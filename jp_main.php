...
function wpcareers_footer($tpl){
  ...
  if(!function_exists('jp_array_sort_by_fields')){
    function jp_array_sort_by_fields(&$data, $sortby){
      ...
      uasort( $data,
      create_function('$a, $b', '$skeys=explode(\',\',\''.$sortby.'\');
  ...
  $jLastPosts=$wpdb->get_results($sql);
  $new_jobs=array();
  for ($l=0; $l<count($jLastPosts); $l++){
    $result=$jLastPosts[$l];
  ...
  $rLastPosts=$wpdb->get_results($sql);
  $new_resumes=array();
  for ($l=0; $l<count($rLastPosts); $l++){
    $result=$rLastPosts[$l];
